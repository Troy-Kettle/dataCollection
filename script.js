...
function getModerateConcernThreshold(vitalSign, direction) {
    const values = vitalSign.getValues().map(parseFloat);

    if (vitalSign.hasBothDirections) {
        if (direction === 'high') {
            return values[5]; // defaultValues[5], start of moderate concern (high)
        } else if (direction === 'low') {
            return values[2]; // defaultValues[2], end of moderate concern (low)
        }
    } else {
        if (vitalSign.reverseScale) {
            // Worse values are lower
            if (direction === 'low') {
                return values[1]; // defaultValues[1], start of moderate concern (low)
            }
        } else {
            // Worse values are higher
            if (direction === 'high') {
                return values[1]; // defaultValues[1], start of moderate concern (high)
            }
        }
    }
    return null;
...
vitalSignsData.forEach(vitalSign => {
    ...
    vitalSignMap[vitalSign.name] = vitalSign;
});
...
// Part 3: Combinations of vital signs using user's data
const combinations = [
    {
        'Heart Rate': { value: 105, abnormal: true },
        'Systolic Blood Pressure': { value: '100/60', abnormal: true },
        'Respiratory Rate': { value: 18, abnormal: false },
        'Temperature': { value: 36.5, abnormal: false },
        'Oxygen Saturations': { value: 98, abnormal: false }
    },
    {
        'Heart Rate': { value: 105, abnormal: true },
        'Systolic Blood Pressure': { value: '135/80', abnormal: false },
        'Respiratory Rate': { value: 22, abnormal: true },
        'Temperature': { value: 36.5, abnormal: false },
        'Oxygen Saturations': { value: 98, abnormal: false }
    },
    {
        ...
        'Systolic Blood Pressure': { value: '135/80', abnormal: false },
        'Respiratory Rate': { value: 18, abnormal: false },
        'Temperature': { value: 38.1, abnormal: true },
        'Oxygen Saturations': { value: 98, abnormal: false }
    },
    {
        'Heart Rate': { value: 105, abnormal: true },
        ...
        'Respiratory Rate': { value: 18, abnormal: false },
        'Temperature': { value: 36.5, abnormal: false },
        'Oxygen Saturations': { value: 94, abnormal: true }
    },
    {
